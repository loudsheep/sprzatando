...
class OfferDetailsController extends Controller
{
    ...
    {
        if($offer->is_banned && ($request->user() === null || $request->user()->role === "user") || $offer->is_done) {
            abort(404);
        }
        ...
        $currentUserInterestedInOffer = $isLoggedIn && $offer->usersInterested()->where('users.id', '=', $user->id)->get()->count() > 0;
